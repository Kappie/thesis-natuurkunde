\begin{abstract}
\noindent In this chapter, we introduce the central concepts in critical phenomena and finite-size scaling,
...
\subsection{Finite-size scaling}\label{sec:finite_size_scaling}
...
\begin{equation}\label{eq:theta_nu_relation}
  \theta = \frac{1}{\nu}.
\end{equation}
...
\begin{equation}\label{eq:finite_size_scaling_at_Tc}
...
As a side note, we mention that the most generally valid ansatz is actually
\begin{equation}\label{eq:finite_size_scaling_ansatz_tstarN}
  M(T, N) = N^{-\beta/\nu} \mathcal{G}(t^{\star}(N) N^{1/\nu})
\end{equation}
with
\begin{equation}
  t^{\star}(N) = T - T^{\star}(N).
...
At $T_c$, we recover \autoref{eq:finite_size_scaling_at_Tc}. To see this, consider that at $T_c$
\begin{equation}
  t^{\star}(N) = T^{\star}(N) - T_c = a N^{-\lambda} = a N^{-1/\nu},
\end{equation}
where in the final equality we have used \autoref{eq:theta_nu_relation}, so that we have
\begin{equation}
  M(T = T_c, N) = N^{-\beta/\nu}\mathcal{G}(a N^{-1/\nu}N^{1/\nu}) \propto N^{-\beta/\nu}.
\end{equation}

In this thesis, we will mostly work with \autoref{eq:finite_size_scaling_ansatz_tN},
but return to \autoref{eq:finite_size_scaling_ansatz_tstarN} when the value of $T_c$ is not exactly known.
