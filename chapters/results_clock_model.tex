...
\todo[inline]{Change in description what actually the system sizes used are (for finite-N case)}
\todo[inline]{Say what length scale is used in case of finite-$m$}

\begin{figure}
  \centering
  \includegraphics[]{t1_fit_q5.tikz}
  \caption{For $q = 5$, we find $T_1 = 0.917$, fitting the final 6 points.
  Max truncation error = $10^{-6}$, TolX = $10^{-6}$}\label{figure:t1_fit_q5}
\end{figure}
...
\begin{figure}
  ...
  \caption{$T_2 = 0.935$, fitted the last six points ($n = 85,
  90, \dots, 110$).
  max trunc error = $10^{-6}$.}\label{figure:t2_fit_q5_finite_N}
\end{figure}
...
\begin{figure}
  ...
  \caption{
  For the finite-$m$ simulations, the values $m = 20, 25, \dots,
  90$ with a convergence threshold of $10^{-7}$ were used. The fit yields $T_2 = 0.944$.
...
Possible simulations left to run:
\begin{itemize}
  \item $q = 5, 6$ finite-$m$ for locating $T_1$.
  Just don't care about symmetry breaking -- this means that for some values of $m$,
  it will be very hard to locate a maximum, but we just hope those values don't come up.
  \item zoom a little bit past 0.95 to see whether massless behaviour still holds.
\end{itemize}
