...
\begin{equation}
  M(m \to \infty) = M(m_{\text{max}}) + \sum_{m = m_{\text{max}} + 1}^{\infty} \Delta Q_{\text{step}}(m),
\end{equation}
...
set to $10^{-7}$ for values of $m$ below, say, 40, and to $10^{-8}$ for $m$-values above 40.
...
of about $1.1\%$ in the correlation length, and less than $0.3\%$ in the order parameter,
...
\todo[inline]{Similar table for some $N$-values?}
